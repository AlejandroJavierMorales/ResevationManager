...
const GridComponent = ({ reservations = [] }) => {
  ...
  // Generar objeto que mapea cada celda a su color correspondiente y al código de reserva si es la primera celda de la reserva
  const cellData = {};
  reservations.forEach((reservation, index) => {
    ...
    for (let currentDate = new Date(startDate); currentDate <= endDate; currentDate.setDate(currentDate.getDate() + 1)) {
      ...
      if (!cellData[key]) {
        // Si es la primera celda de la reserva, almacenar el código de reserva
        cellData[key] = {
          color,
